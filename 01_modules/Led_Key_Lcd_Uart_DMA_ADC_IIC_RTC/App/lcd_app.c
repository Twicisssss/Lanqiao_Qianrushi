...
void lcd_task(void)
{
    ...
    lcd_sprintf(Line4,"AT24C02[0]:0x%02X",at_test_read_data[0]);                //AT24C02存储值
    lcd_sprintf(Line5,"MCP4017:%d (0x%02X)   ", mcp4017_value, mcp4017_value);  //MCP4017当前值
    
    lcd_sprintf(Line7,"Time:%2d-%2d-%2d",time.Hours,time.Minutes,time.Seconds);    
    lcd_sprintf(Line8,"Date:%2d-%2d-%2d",date.Year,date.Month,date.Date);    
}
